...
class CommunityService {
  ...
  static async attachProfilesToPosts(posts) {
    ...
    posts.forEach((p) => {
      ...
      if (Array.isArray(p.repostsDetails)) {
        p.repostsDetails.forEach((r) => {
          if (r.repostedBy && r.repostedBy._id)
            userIds.add(r.repostedBy._id.toString());
        });
      }
      if (Array.isArray(p.sharesDetails)) {
        p.sharesDetails.forEach((s) => {
          if (s.sharedBy && s.sharedBy._id)
            userIds.add(s.sharedBy._id.toString());
        });
      }
    });

    const ids = Array.from(userIds).map(
    ...
    return posts.map((p) => {
      ...
      if (Array.isArray(obj.comments)) {
        obj.comments = obj.comments.map((c) => ({
          ...c,
          authorProfile: attach(c.author),
        }));
      }
      if (Array.isArray(obj.repostsDetails)) {
        obj.repostsDetails = obj.repostsDetails.map((r) => ({
          ...r,
          repostedByProfile: attach(r.repostedBy),
        }));
      }
      if (Array.isArray(obj.sharesDetails)) {
        obj.sharesDetails = obj.sharesDetails.map((s) => ({
          ...s,
          sharedByProfile: attach(s.sharedBy),
        }));
      }
      return obj;
    });
  ...
  static async createPost({ userId, content, image, visibility = "public" }) {
    ...
    const post = await this.basePostPopulate(Post.findById(newPost._id))
      .lean()
      .exec();
  ...
  static async getPostById(postId, viewerId = null) {
    ...
    const populated = await this.basePostPopulate(Post.findById(postId))
      .lean()
      .exec();
  ...
  static async getUserPostsAndReposts(userId, { page = 1, limit = 10 } = {}) {
    ...
    const posts = await this.basePostPopulate(
      Post.find({
        $or: [{ author: userId }, { reposts: userId }, { shares: userId }],
      })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit),
  ...
  static async repostPost(postId, userId) {
    ...
    const isAlreadyReposted = post.reposts.some(
      (id) => id.toString() === userId.toString(),
    );
  ...
  static async updatePost(postId, userId, updatedData) {
    ...
    if (updatedData.visibility !== undefined)
      post.visibility = updatedData.visibility;
  ...
  static async followUser(followerId, followingId) {
    ...
    try {
      ...
      const follower = await UserProfileModel.findOne({ followerId })
        .select("username")
        .lean()
        .exec();
