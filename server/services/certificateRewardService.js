...
class CertificateRewardService {
  static async computeUserStats(userId) {
    ...
    for (const category in transportTypes) {
      ...
      for (const subtype of transportTypes[category])
        totals.subtypes[subtype] = 0;
    }

    const findCategory = (subtype) => {
      for (const category in transportTypes)
        if (transportTypes[category].includes(subtype)) return category;
      return null;
    };
    ...
    for (const a of activities) {
      ...
      if (a.createdAt)
        totals.dates.push(a.createdAt.toISOString().split("T")[0]);
    }
    ...
    const userInfo =
      (await LeaderboardService.getUserLeaderboardInfo(userId)) || null;
  ...
  static computeStreak(datesSorted) {
    ...
    if (!dateSet.has(todayStr))
      current = new Date(datesSorted[datesSorted.length - 1]);
  ...
  static async checkCertificatesForUser(userId, stats = null) {
    ...
    for (const cert of certificates) {
      const exists = await UserCertificate.findOne({
        userId,
        certificateId: cert._id,
      });
      if (exists) continue;

      const req = this.parseRequirement(
        cert.requirement || cert.unlockRequirement,
      );
      if (req && this.requirementSatisfied(req, mergedStats)) {
        const doc = new UserCertificate({
          userId,
          certificateId: cert._id,
          unlockedAt: new Date(),
        });
  ...
  static computeProgress(requirement, stats) {
    ...
    const checkers = {
      ...
      motorizeddistance: (s) =>
        (s.categories?.private || 0) + (s.categories?.public || 0),
      activedistance: (s) =>
        (s.categories?.private || 0) +
        (s.categories?.public || 0) +
        (s.categories?.basic || 0),
    };
  ...
  static async getUserCertificates(userId) {
    ...
    const userCerts = await UserCertificate.find({ userId }).populate(
      "certificateId",
    );
    const allCerts = await Certificate.find();

    return allCerts.map((cert) => {
      const unlocked = userCerts.find(
        (uc) =>
          uc.certificateId && String(uc.certificateId._id) === String(cert._id),
      );
      const req = this.parseRequirement(
        cert.requirement || cert.unlockRequirement,
      );
  ...
  static async getUserRewards(userId) {
    ...
    const map = new Map(
      userRewards
        .filter((ur) => ur.rewardId)
        .map((ur) => [String(ur.rewardId._id), ur]),
    );
    ...
    return allRewards.map((r) => {
      ...
      return {
        ...
        claimRequirement:
          r.claimRequirement ||
          (r.requirement ? `${r.requirement.type}_${r.requirement.value}` : ""),
        progress: Number((progress * 100).toFixed(1)),
      };
    });
  }
