    ...
    try {
      ...
      const postWithFlags = {
        ...
        isLiked: userId
          ...
          : false,
        isReposted: userId
          ? obj.reposts.some((id) => id.toString() === userId)
          : false,
        isShared: userId
          ? obj.shares.some((id) => id.toString() === userId)
          : false,
  ...
  try {
    ...
    const postsWithFlags = posts.map((post) => ({
      ...
      isLiked: userId
        ...
        : false,
      isReposted: userId
        ? post.reposts?.some((id) => id.toString() === userId)
        : false,
      isShared: userId
        ? post.shares?.some((id) => id.toString() === userId)
        : false,
...
router.get("/posts/feed/following", async (req, res) => {
  try {
    ...
    const { posts, hasMore } = await CommunityService.getFollowingFeed(userId, {
      page,
      limit,
    });
...
router.post("/posts/:id/repost", async (req, res) => {
  try {
    const updatedPost = await CommunityService.repostPost(
      req.params.id,
      req.user._id,
    );
    const userId = req.user._id.toString();
...
router.post("/posts/:id/share", async (req, res) => {
  try {
    const updatedPost = await CommunityService.sharePost(
      req.params.id,
      req.user._id,
    );
    const userId = req.user._id.toString();
...
router.patch(
  "/posts/:id",
  multer.uploadCommunityImage.single("image"),
  async (req, res) => {
    try {
      ...
      if (!content && !image && visibility === undefined) {
        return res
          .status(400)
          .json({
            error: "No content, image, or visibility provided for update",
          });
      }

      const updatedPost = await CommunityService.updatePost(postId, userId, {
        content,
        image,
        visibility,
      });
...
router.get("/followers/:userId", async (req, res) => {
  try {
    ...
    const followers = await CommunityService.getFollowers(req.params.userId, {
      page,
      limit,
    });
    res.json(followers);
...
router.get("/following", async (req, res) => {
  try {
    const page = parseInt(req.query.page || "1", 10);
    const limit = parseInt(req.query.limit || "20", 10);
    const following = await CommunityService.getFollowingList(req.user._id, {
      page,
      limit,
    });
    res.json(following);
