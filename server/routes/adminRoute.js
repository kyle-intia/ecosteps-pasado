...
router.get("/dashboard/dailytrackings/user/:userId", async (req, res) => {
  try {
    const tracking = await AdminService.getDailyTrackingByUserId(
      req.params.userId,
    );
    if (!tracking) {
      return res
        .status(404)
        .json({ error: "Daily tracking not found for this user" });
    }
    res.json(tracking);
  } catch (err) {
...
router.patch("/dashboard/dailytrackings/:id", async (req, res) => {
  try {
    const updated = await AdminService.patchDailyTracking(
      req.params.id,
      req.body,
    );
    res.json(updated);
...
router.get("/dashboard/emission/daily/:date", async (req, res) => {
  try {
    const data = await AdminService.getDailyFootprintByCategory(
      req.params.date,
    );
    res.json(data);
  } catch (error) {
...
router.get("/dashboard/emission/monthly/:month", async (req, res) => {
  try {
    const data = await AdminService.getMonthlyFootprintByCategory(
      req.params.month,
    );
    res.json(data);
  } catch (error) {
...
router.get("/dashboard/emission/yearly/:year", async (req, res) => {
  try {
    const data = await AdminService.getYearlyFootprintByCategory(
      req.params.year,
    );
    res.json(data);
  } catch (error) {
...
router.get("/users/admin", async (req, res) => {
  try {
    const admins = await AdminService.getUserAdmin();
...
router.patch("/users/:id/status", async (req, res) => {
  try {
    const user = await AdminService.updateStatus(
      req.params.id,
      req.body.status,
    );
    res.json(user);
