...
if (hour >= 9) {
  sendDailyTrackingReminders()
    .then(() =>
      console.log(
        "Reminder sent on server start (aligned with 4-hour schedule).",
      ),
    )
    .catch((err) =>
      console.error("Failed to send reminder on server start:", err),
    );
}
...
const startServer = async () => {
  try {
    ...
    server.listen(env_1.PORT, "0.0.0.0", () => {
      console.log(
        `Server is running on port ${env_1.PORT} in ${env_1.NODE_ENV} mode`,
      );
