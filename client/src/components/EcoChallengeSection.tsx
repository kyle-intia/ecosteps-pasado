// client/src/components/EcoChallengeSection.tsx
// Updated component with daily tracking validation and improved UX
...
const EcoChallengeSection: React.FC = () => {
  ...
  // Fetch today's challenges on component mount
  useEffect(() => {
  ...
  const regenerateChallenges = async () => {
    try {
      ...
      toast({
        ...
        description: "Today's challenges have been refreshed with new selections.",
        variant: "default"
      });
    } catch (error) {
  ...
  const completeChallenge = async (challengeId: string) => {
    try {
      ...
      if (!response.ok) {
        // Handle specific error for missing daily tracking
        if (result.message?.includes('daily tracking')) {
          toast({
            title: "Daily Tracking Required",
            description: "Please complete your daily carbon footprint tracking first before attempting eco-challenges.",
            variant: "destructive"
          });
          return;
        }
      ...
      // Update the challenge data with the completion
      if (challengeData) {
      ...
      // Show success toast with recalculation info
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to complete challenge. Please try again.",
        variant: "destructive"
      });
    } finally {
      setCompleting(null);
  ...
  // Show tracking requirement warning if user hasn't completed daily tracking
  if (challengeData.trackingRequired) {
    return (
      <div className="mb-8">
        ...
        {/* Daily Tracking Required Alert */}
        <Alert className="mb-6 border-warning bg-warning/10">
          <Lock className="h-4 w-4" />
          <AlertDescription className="flex items-center justify-between">
            <div>
              <strong>Daily Tracking Required:</strong> Complete your daily carbon footprint tracking first to unlock eco-challenges!
            </div>
            <Button 
              size="sm" 
        ...
        <Card className="mt-6 shadow-card border-border">
          <CardContent className="p-6 text-center">
            <div className="mb-4">
              ...
              <h3 className="text-lg font-semibold mb-2">Unlock Your Eco-Challenges</h3>
              <p className="text-muted-foreground mb-4">
                Track your daily carbon footprint to unlock today's personalized eco-challenges and start making a difference!
              </p>
              <Button 
                onClick={() => navigate('/track')}
        ...
        <CardHeader>
          ...
          {challengeData.isRecalculated && (
            <div className="flex items-center space-x-2 text-sm text-success">
              ...
              <span>Your carbon footprint has been updated with challenge savings!</span>
            </div>
          )}
        </CardHeader>
      </Card>
          ...
          {challengeData.allCompleted && (
            <div className="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
              <p className="text-sm text-green-800">
                <strong>Congratulations!</strong> You've completed all three eco-challenges today. 
                Your actions are making a real difference for the planet!
              </p>
            </div>
          )}
        </CardContent>
