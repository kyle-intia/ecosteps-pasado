...
import {
  AssessmentDialog,
  useAssessmentDialog,
} from "@/components/AssessmentDialog";
...
const Home = () => {
  ...
  const { showAssessment, assessmentType, handleComplete } =
    useAssessmentDialog(userId);

  if (isLoading) return <Spinner />;
  if (isError) return <p>Error: {String(profileError)}</p>;
  ...
  const ecoScore = leaderboardError
    ? "N/A"
    : (userLeaderboard?.totalScore ?? "N/A");
  const currentRank = leaderboardError
    ? "N/A"
    : (userLeaderboard?.currentRank ?? "N/A");
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-sm text-foreground">
                          <strong>@{highlight.username}</strong>{" "}
                          {highlight.content}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {formatDistanceToNow(highlight.timestamp, {
                            addSuffix: true,
                          })}
                        </p>
                      </div>
