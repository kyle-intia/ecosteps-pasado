...
import { getDailyTrackingHistory, getChallengeHistory, getUserCommunityPosts } from "../lib/api";
...
const useRecentActivities = (limit = 10) => {
  ...
  // Safely process challenge data
  if (challengeData?.data?.history && Array.isArray(challengeData.data.history)) {
    challengeData.data.history.forEach(day => {
      if (day.challenges && Array.isArray(day.challenges)) {
        day.challenges.filter(c => c.completed).forEach(challenge => {
          activities.push({
            type: 'challenge',
  ...
  if (postsData && Array.isArray(postsData)) {
    postsData.slice(0, limit).forEach(post => {
      activities.push({
        ...
        description: post.content ? (post.content.substring(0, 50) + (post.content.length > 50 ? '...' : '')) : 'Shared a post',
  ...
  return {
    ...
    isLoading: trackingRest.isLoading || challengeRest.isLoading || postsRest.isLoading,
